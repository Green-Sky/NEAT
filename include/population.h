...
namespace NEAT {
	...
	public:
		...
		Population(NEAT& neat, Genome *g, int size);

		// Construct off of a single spawning Genome without mutation
		Population(NEAT& neat, Genome *g, int size, float power);

		//MSC Addition
		// Construct off of a vector of genomes with a mutation rate of "power"
		Population(NEAT& neat, std::vector<Genome*> genomeList, float power);

		bool clone(NEAT& neat, Genome *g, int size, float power);
		...
		Population(NEAT& neat, const char *filename);
