...
namespace NEAT {
	...
	public:
        ...
        Genome(NEAT& neat, int id, std::ifstream &iFile);
		...
		Genome(NEAT& neat, int new_id,int i, int o, int n,int nmax, bool r, double linkprob);
		...
		static Genome *new_Genome_load(NEAT& neat, char *filename);
		...
		void mutate_random_trait(NEAT& neat);

		// Change random link's trait. Repeat times times
		void mutate_link_trait(NEAT& neat, int times);

		// Change random node's trait times times
		void mutate_node_trait(NEAT& neat, int times);

		// Add Gaussian noise to linkweights either GAUSSIAN or COLDGAUSSIAN (from zero)
		void mutate_link_weights(NEAT& neat, double power,double rate,mutator mut_type);

		// toggle genes on or off
		void mutate_toggle_enable(NEAT& neat, int times);

		// Find first disabled gene and enable it
		void mutate_gene_reenable(NEAT& neat);
		...
		bool mutate_add_node(NEAT& neat, std::vector<Innovation*> &innovs,int &curnode_id,double &curinnov);

		// Mutate the genome by adding a new link between 2 random NNodes
		bool mutate_add_link(NEAT& neat, std::vector<Innovation*> &innovs,double &curinnov,int tries);

		void mutate_add_sensor(NEAT& neat, std::vector<Innovation*> &innovs, double &curinnov);
		...
		Genome *mate_multipoint(NEAT& neat, Genome *g,int genomeid,double fitness1, double fitness2, bool interspec_flag);
		...
		//   weights
		Genome *mate_multipoint_avg(NEAT& neat, Genome *g,int genomeid,double fitness1,double fitness2, bool interspec_flag);
		...
		//   with the bigger one.
		Genome *mate_singlepoint(NEAT& neat, Genome *g,int genomeid);
		...
		void randomize_traits(NEAT& neat);
